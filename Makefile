...
PHYSICAL_PAGE_ALLOCATOR_SRC = $(KERNEL_MEMORY_DIR)/physical_page_allocator.cpp
KERNEL_HEAP_SRC = $(KERNEL_MEMORY_DIR)/kernel_heap.cpp
...
PHYSICAL_PAGE_ALLOCATOR_OBJ = $(BUILD_DIR)/physical_page_allocator.o
KERNEL__HEAP_OBJ = $(BUILD_DIR)/kernel_heap.o
...
$(PHYSICAL_PAGE_ALLOCATOR_OBJ): $(PHYSICAL_PAGE_ALLOCATOR_SRC) | $(BUILD_DIR)
	$(CXX) $(CXXFLAGS) $(PHYSICAL_PAGE_ALLOCATOR_SRC) -o $(PHYSICAL_PAGE_ALLOCATOR_OBJ)

$(KERNEL__HEAP_OBJ): $(KERNEL_HEAP_SRC) | $(BUILD_DIR)
	$(CXX) $(CXXFLAGS) $(KERNEL_HEAP_SRC) -o $(KERNEL__HEAP_OBJ)

kernel: $(KERNEL_ASM_OBJ) $(KERNEL_MAIN_OBJ) $(TERMINAL_OBJ) $(MEMORY_OBJ) $(PANIC_OBJ) $(LOGGER_OBJ) $(INTERRUPT_STUBS_OBJ) $(IDT_OBJ) $(INTERRUPTS_OBJ) $(EXCEPTIONS_OBJ) $(IO_OBJ) $(PIC_OBJ) $(KEYBOARD_OBJ) $(CPU_OBJ) $(STRING_OBJ) $(CONSOLE_OBJ) $(SHELL_OBJ) $(MEMORY_MAP_OBJ) $(PHYSICAL_MEMORY_OBJ) $(PAGE_OBJ) $(PHYSICAL_PAGE_ALLOCATOR_OBJ) $(KERNEL__HEAP_OBJ)
	$(LD) -T $(LINKER_SCRIPT) -o $(KERNEL_ELF) $(KERNEL_ASM_OBJ) $(KERNEL_MAIN_OBJ) $(TERMINAL_OBJ) $(MEMORY_OBJ) $(PANIC_OBJ) $(LOGGER_OBJ)  $(INTERRUPT_STUBS_OBJ) $(IDT_OBJ) $(INTERRUPTS_OBJ) $(EXCEPTIONS_OBJ) $(IO_OBJ) $(PIC_OBJ) $(KEYBOARD_OBJ) $(CPU_OBJ) $(STRING_OBJ) $(CONSOLE_OBJ) $(SHELL_OBJ) $(MEMORY_MAP_OBJ) $(PHYSICAL_MEMORY_OBJ) $(PAGE_OBJ) $(PHYSICAL_PAGE_ALLOCATOR_OBJ) $(KERNEL__HEAP_OBJ)
